<template>
  <div :class="`item-menu ${this.item.class ?? ''}`" :data-indice="this.getIndice">
    <RouterLink v-if="this.item.to" class="conteudo-item-menu" :to="this.item.to" :title="this.item.title"
      @click.prevent="this.expanded">
    ...
    <div v-if="!this.item.to && this.item.itens" class="conteudo-item-submenu">
      <ComponenteMenuItem v-if="this.item.itens" v-for="(itemSubMenu, indiceMenu) in this.item.itens" :item="itemSubMenu"
        :indice="indiceMenu" :indiceStart="this.getIndice" />
    </div>
  </div>
</template>
...
<style lang="scss">@import '../assets/styles/components/ComponenteMenuItem.scss';</style>